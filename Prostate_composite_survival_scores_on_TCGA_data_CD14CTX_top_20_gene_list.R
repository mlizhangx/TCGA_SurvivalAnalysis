...
metadata_count$sample_id <- substr(metadata_count$sample_id,1,15)


# load survival info
survival_info <- read.csv("./data/prad_tcga_clinical_data.csv") #,sep = "\t"
...
colnames(read_gene_list) <- c("gene","logfoldchange","padj")
...
head(read_gene_list$names, n=Top_N)
...
#### KM plot over all ####
pdf(paste0("./figures/Prostate KM plot on median Composite survival score ", cluster_id, " the most positive fold change in the RvNR ", Top_N," gene list plot.pdf"))
...
legend(x=13,y=1,legend=paste0("Median Disease_Free: ", round(fit_summary_table[7,1], 2), " months, 95% CI (", round(fit_summary_table[8,1], 2), ", ", round(fit_summary_table[9,1], 2), " ) (n=", nrow(surv_merge_info), ")"),lty=1,bty="n",cex=1) 
...
legend(x=21,y=1.02,legend=paste0("Median Disease_Free for CS > median: ", round(median(fit_summary_table[2,7]), 1), " months"),col="red",lty="solid",bty="n",cex=1.0)
legend(x=21,y=0.98,legend=paste0("Median Disease_Free for CS < median: ", round(median(fit_summary_table[1,7]), 1), " months"),col="black",lty="dashed",bty="n",cex= 1.0)
dev.off()
